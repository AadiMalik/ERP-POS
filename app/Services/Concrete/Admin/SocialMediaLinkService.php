...
class SocialMediaLinkService
{
    ...
    {
        $this->model_link = new Repository(new SocialMediaLink());
    }
        ...
        return DataTables::of($datatable)
            ->addColumn('status', function ($item) {
                ...
                <div class="form-check form-switch mb-0">
                    <input class="form-check-input statusSocialMedia" type="checkbox" data-id="' . $item->social_media_link_id . '" ' . $checked . '>
                </div>';
            })
            ->addColumn('preview', function ($item) {
                $color = $item->display_color ?: '#666';
                $icon = $item->icon ?: 'fa fa-link';
                return '<i class="' . e($icon) . '" style="color:' . e($color) . ';font-size:20px;"></i>';
            })
            ->addColumn('action', function ($item) {
                return "
                    <a class='btn btn-icon btn-outline-primary mr-2' id='editSocialMedia' href='javascript:void(0)' data-id='{$item->social_media_link_id}' data-original-title='Edit'><i class='icon-base fa fa-pencil'></i></a>
                    <a class='btn btn-icon btn-outline-danger' id='deleteSocialMedia' data-id='{$item->social_media_link_id}'><i class='fa fa-trash'></i></a>
                ";
            })
    ...
    {
        if (!empty($obj['social_media_link_id'])) {
            $obj['updatedby_id'] = Auth::id();
            $obj['date_updated'] = now();
            $this->model_link->update($obj, $obj['social_media_link_id']);
            return $this->model_link->find($obj['social_media_link_id']);
        }

        $obj['social_media_link_id'] = generateUuid();
