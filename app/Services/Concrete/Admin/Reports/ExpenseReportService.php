...
/**
 * Account-wise summary of every Expense-type Chart of Accounts entry
 * (Cost of Goods Sold, Direct Expenses, Operating Expenses, etc.) for a
 * period, read entirely from posted journal_entry_details.
 */
class ExpenseReportService
{
    ...
    public function build(array $obj)
    {
        ...
        $accountsQuery = Account::with(['accountType', 'accountSubType'])
            ->whereHas('accountType', fn ($q) => $q->where('code', AccountTypes::CODES[AccountTypes::EXPENSES]))
            ->where('is_deleted', 0)
        ...
        if (!empty($obj['account_sub_type_id'])) {
            $accountsQuery->where('account_sub_type_id', $obj['account_sub_type_id']);
        }
        ...
        foreach ($accounts as $account) {
            ...
            $netAmount = round($period['debit'] - $period['credit'], 2);
