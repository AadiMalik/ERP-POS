...
use App\Services\Concrete\Admin\Reports\PurchaseReturnSummaryReportService;
...
class PurchaseReturnSummaryReportController extends Controller
{
    use ResponseAPI;

    protected $purchase_return_summary_report_service;
    ...
    public function __construct(
        PurchaseReturnSummaryReportService $purchase_return_summary_report_service,
    ...
    public function index()
    {
        ...
        $group_by_options = PurchaseReturnSummaryReportService::GROUP_BY_OPTIONS;

        return view('admin.reports.purchase_return_summary.index', compact('business', 'suppliers', 'warehouses', 'group_by_options'));
    }

    public function data(Request $request)
    {
        return $this->purchase_return_summary_report_service->getData($request->all());
    }

    public function print(Request $request)
    {
        $rows = $this->purchase_return_summary_report_service->build($request->all());
        $business_id = $request->business_id ?? Auth::user()->business_id;

        $this->log($business_id, 'print');

        return view('admin.reports.purchase_return_summary.print.print', compact('rows', 'request'));
    }
    ...
    public function pdf(Request $request)
    {
        ...
        return Pdf::loadView('admin.reports.purchase_return_summary.pdf', compact('rows', 'request'))
            ->setPaper($print_config->page('paper_size', 'a4'), $print_config->page('orientation', 'portrait'))
            ->stream('purchase-return-summary.pdf');
    }
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new PurchaseReturnSummaryExport($rows), 'purchase-return-summary.xlsx');
    }
    ...
    public function exportCsv(Request $request)
    {
        ...
        return Excel::download(new PurchaseReturnSummaryExport($rows), 'purchase-return-summary.csv', ExcelFormat::CSV);
    }

    protected function log(string $business_id, string $channel): void
    {
        try {
            $this->document_send_log_service->log($business_id, 'purchase_return_summary_report', $business_id, $channel, null, 'sent', null, Auth::id());
        } catch (Exception $e) {
