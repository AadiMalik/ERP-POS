...
use App\Services\Concrete\Admin\Reports\Inventory\StockTransferReportService;
use App\Services\Concrete\Admin\WarehouseService;
use Illuminate\Http\Request;

class StockTransferReportController extends BaseInventoryReportController
{
    public function __construct(
        StockTransferReportService $service,
    ...
    protected function permissionName(): string { return 'reports.stock-transfer-report.view'; }
    protected function reportKey(): string { return 'stock-transfer-report'; }
    protected function viewDir(): string { return 'stock_transfer'; }

    protected function exportInstance($rows)
    {
        return new InventoryReportExport(
            $rows,
            ['Transfer No', 'Date', 'Source WH', 'Dest WH', 'Source Branch', 'Dest Branch', 'Qty', 'Value', 'Status'],
            ['transfer_note_no', 'transfer_note_date', 'source_warehouse', 'destination_warehouse', 'source_branch', 'destination_branch', 'total_quantity', 'total_value', 'status']
        );
    }
    ...
    public function index(Request $request)
    {
        ...
        $report_title = 'Stock Transfer Report';
        $preset_mode = $request->get('report_mode');

        return view('admin.reports.inventory.stock_transfer.index', compact(
