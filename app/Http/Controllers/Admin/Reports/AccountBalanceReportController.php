...
use App\Services\Concrete\Admin\Reports\AccountBalanceReportService;
...
class AccountBalanceReportController extends Controller
{
    ...
    protected $account_balance_report_service;
    ...
    public function __construct(
        AccountBalanceReportService $account_balance_report_service,
    ...
    public function index()
    {
        ...
        return view('admin.reports.account_balance.index', compact('business', 'account_types'));
    }

    public function data(Request $request)
    {
        return $this->account_balance_report_service->getData($request->all());
    }

    public function print(Request $request)
    {
        $rows = $this->account_balance_report_service->build($request->all());
        $business_id = $request->business_id ?? Auth::user()->business_id;

        $this->log($business_id, 'print');

        return view('admin.reports.account_balance.print.print', compact('rows', 'request'));
    }
    ...
    public function pdf(Request $request)
    {
        ...
        return Pdf::loadView('admin.reports.account_balance.pdf', compact('rows', 'request'))
            ->setPaper($print_config->page('paper_size', 'a4'), $print_config->page('orientation', 'portrait'))
            ->stream('account-balance.pdf');
    }
    ...
    public function export(Request $request)
    {
        ...
        return Excel::download(new AccountBalanceExport($rows), 'account-balance.xlsx');
    }
    ...
    public function exportCsv(Request $request)
    {
        ...
        return Excel::download(new AccountBalanceExport($rows), 'account-balance.csv', ExcelFormat::CSV);
    }

    protected function log(string $business_id, string $channel): void
    {
        try {
            $this->document_send_log_service->log($business_id, 'account_balance_report', $business_id, $channel, null, 'sent', null, Auth::id());
        } catch (Exception $e) {
