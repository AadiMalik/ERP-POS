...
<script>
    ...
    var firstSection = document.querySelector('.coa-section-content');
    if (firstSection) {
        firstSection.style.display = 'block';
    }
    document.querySelectorAll('.coa-section-header').forEach(function(header) {
        ...
        header.addEventListener('click', function() {
            ...
            document.querySelectorAll('.coa-section-content').forEach(function(item) {

                if (item != content) {
                    item.style.display = 'none';

                    const id = item.id.replace('sectionContent', '');
                    const i = document.getElementById('sectionIcon' + id);

                    if (i)
                        i.className = 'fa fa-chevron-right expand-icon';
                }
    ...
    document.addEventListener('click', function(e) {
        ...
        // Same parent ke sirf siblings close hon
        const parentContainer = target.parentElement;

        parentContainer.querySelectorAll(':scope > .child-accounts.open').forEach(function(item) {

            if (item.id != target.id) {

                item.classList.remove('open');

                const toggle = parentContainer.querySelector(
                    '.toggle-btn[data-target="' + item.id + '"]'
                );

                if (toggle) {
                    toggle.querySelector('.toggle-icon').className =
        ...
        btn.querySelector('.toggle-icon').className =
            target.classList.contains('open') ?
            'fa fa-chevron-down toggle-icon' :
            'fa fa-chevron-right toggle-icon';

    });

    $(document).ready(function() {
        $('#parent_business_id').select2({
            dropdownParent: $('#parentAccountModal')
        });
        $('#parent_account_type_id').select2({
            dropdownParent: $('#parentAccountModal')
        });
        $('#parent_account_sub_type_id').select2({
            dropdownParent: $('#parentAccountModal')
        });
        //child
        $('#child_business_id').select2({
            dropdownParent: $('#childAccountModal')
        });
        $('#child_account_type_id').select2({
            dropdownParent: $('#childAccountModal')
        });
        $('#child_account_sub_type_id').select2({
            dropdownParent: $('#childAccountModal')
        });
        $('#child_parent_account_id').select2({
            dropdownParent: $('#childAccountModal')
        });
    });
</script>
