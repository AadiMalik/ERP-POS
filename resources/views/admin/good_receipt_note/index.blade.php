            ...
            <div class="card-body">
                <div id="filterSection" class="card-body border-bottom" style="display:none;">
                    <div class="row g-3">
                        ...
                        @endif
                        <div class="col-md-3">
                            <label class="form-label">{{ __('common.purchase') }}</label>
                            <select id="purchase_id" class="form-select">
                                <option value="">{{ __('good_receipt_notes.all_purchases') }}</option>
                                @if (RoleNames::SUPERADMIN != getRoleName())
                ...
                <div class="table-responsive p-4">
                    <table id="grn_table" class="table datatables">
                        <thead>
                            <tr>
                                <th>{{ __('good_receipt_notes.grn_no') }}</th>
                                <th>{{ __('good_receipt_notes.grn_date') }}</th>
                                <th>{{ __('good_receipt_notes.purchase_no') }}</th>
...
@section('js')
    @php
        $__i18nGrn = [
            ...
            'all_purchases' => __('good_receipt_notes.all_purchases'),
        ];
    @endphp
    <script>
        window.i18n_good_receipt_notes = @json($__i18nGrn);
    </script>
    ...
    <script>
        ...
        $(document).on('change', '.change-status', function() {
            ...
            $.ajax({
                ...
                error: function(error) {
                    ...
                    errorMessage(error.responseJSON?.Message || window.i18n_good_receipt_notes?.something_went_wrong || 'Something went wrong.');
                    initDataTablegrn_table();
        ...
        $('#business_id').on('change', function() {
            ...
            $('#supplier_id').html('<option value="">{{ __('common.all_suppliers') }}</option>');
            $('#warehouse_id').html('<option value="">{{ __('common.all_warehouses') }}</option>');
            $('#purchase_id').html('<option value="">{{ __('good_receipt_notes.all_purchases') }}</option>');
            ...
            Promise.all([
                ...
                .then(([supplierRes, warehouseRes, purchaseRes]) => {
                    ...
                    let supplierOptions = '<option value="">{{ __('common.all_suppliers') }}</option>';
                    ...
                    let warehouseOptions = '<option value="">{{ __('common.all_warehouses') }}</option>';
                    ...
                    let purchaseOptions = '<option value="">{{ __('good_receipt_notes.all_purchases') }}</option>';
                ...
                .catch((err) => {
                    errorMessage(err.Message ?? (window.i18n_good_receipt_notes?.something_went_wrong || 'Something went wrong.'));
                });
