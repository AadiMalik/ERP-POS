...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4">{{ isset($service_purchase_return) ? 'Update' : 'New' }} Service Purchase Return</h4>
        <div class="card">
            <div class="card-header bg-white border-bottom">
                <h5 class="mb-0">{{ isset($service_purchase_return) ? 'Update' : 'Create' }} Service Purchase Return</h5>
            </div>
            <div class="card-body">
                <form action="{{ url('admin/service-purchase-return') }}" method="POST" id="servicePurchaseReturnForm">
                    @csrf
                    <input type="hidden" name="service_purchase_return_id"
                        value="{{ $service_purchase_return->service_purchase_return_id ?? '' }}">
                    {{-- ================= HEADER ================= --}}
                    <div class="row">
                                ...
                                <select class="form-control select2" name="business_id" id="business_id">
                                    ...
                                    @foreach ($business as $item)
                                        <option value="{{ $item->business_id }}"
                                            {{ old('business_id', $service_purchase_return->business_id ?? '') == $item->business_id ? 'selected' : '' }}>
                                            {{ $item->code }} - {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-3 mb-3">
                            <label>
                                Service Purchase<span class="text-danger">*</span>
                            </label>
                            <select class="form-control select2" name="service_purchase_id" id="service_purchase_id"
                                {{ isset($service_purchase_return) ? 'disabled' : '' }}>
                                <option value="">--Select Service Purchase--</option>
                                @foreach ($service_purchases as $item)
                                    <option value="{{ $item->service_purchase_id }}"
                                        {{ old('service_purchase_id', $service_purchase_return->service_purchase_id ?? '') == $item->service_purchase_id ? 'selected' : '' }}>
                                        {{ $item->service_purchase_no }} - {{ $item->supplier->name ?? '' }}
                                    </option>
                                @endforeach
                            </select>
                            @if (isset($service_purchase_return))
                                <input type="hidden" name="service_purchase_id"
                                    value="{{ $service_purchase_return->service_purchase_id }}">
                            @endif
                        </div>
                        <div class="col-md-3 mb-3">
                            <label>Supplier</label>
                            <input type="text" class="form-control" id="supplier_name" readonly
                                value="{{ $service_purchase_return->supplier->name ?? '' }}">
                        </div>
                        <div class="col-md-3 mb-3">
                            <label>Return Number</label>
                            <input type="text" class="form-control" name="service_purchase_return_no" readonly
                                value="{{ $service_purchase_return->service_purchase_return_no ?? ($service_purchase_return_no ?? 'Auto Generated') }}">
                        </div>
                        <div class="col-md-3 mb-3">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control" name="reason"
                                value="{{ old('reason', $service_purchase_return->reason ?? '') }}">
                        </div>
                        <div class="col-md-12">
                            <label>Description</label>
                            <textarea class="form-control" rows="3" name="description">{{ old('description', $service_purchase_return->description ?? '') }}</textarea>
                        </div>
                    </div>
                    <hr>
                    {{-- ================= ITEM TABLE ================= --}}
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <button class="text-end btn btn-primary" id="submitBtn">
                                {{ isset($service_purchase_return) ? '{{ __('service_purchase_returns.update_heading') }}' : 'Save Service Purchase Return' }}
                            </button>
                        </div>
                    </div>
                </form>
...
@section('js')
    ...
    @endif
    <script>
        var isEditMode = {{ isset($service_purchase_return) ? 'true' : 'false' }};
        var editReturnData = @json($service_purchase_return_details ?? null);
        ...
        $(function() {
            ...
            if (isEditMode) {
                ...
                let sourceId = $('#service_purchase_id').val();
                if (sourceId) {
        ...
        $(document).on('change', '#service_purchase_id', function() {
            let service_purchase_id = $(this).val();

            if (!service_purchase_id) {
                resetItemRows();
                return;
            }

            loadSourceLines(service_purchase_id);
        });
        ...
        function loadSourceLines(service_purchase_id) {
            $.ajax({
                url: url_local + '/admin/service-purchase-return/source-lines/' + service_purchase_id,
                type: 'GET',
                dataType: 'json',
                beforeSend: function() {
                    $('#itemRows').html(`
        ...
        function bindItemRows(lines) {
            ...
            if (!lines || !lines.length) {
                $('#itemRows').html(`
                    <tr id="emptyRow">
                        <td colspan="9" class="text-center text-muted">
                            Nothing remaining to return for this service purchase.
                        </td>
                    </tr>
                `);
                calculateGrandTotal();
        ...
        function addItemRow(line, return_quantity) {
            ...
            let row = $(`
                <tr class="item-row">
                    <td>
                        <input type="hidden" name="items[][service_purchase_detail_id]" value="${line.service_purchase_detail_id}">
            ...
            row.find('input[name*="[service_purchase_detail_id]"]').attr('name',
                `items[${$('#itemRows tr.item-row').length}][service_purchase_detail_id]`);
        ...
        function reindexRows() {
            $('#itemRows tr.item-row').each(function(index) {
                $(this).find('input[name*="[service_purchase_detail_id]"]').attr('name',
                    `items[${index}][service_purchase_detail_id]`);
        ...
        function loadReturnForEdit() {
            ...
            $.each(editReturnData.items, function(_, item) {
                addItemRow({
                    service_purchase_detail_id: item.service_purchase_detail_id,
        ...
        $('#servicePurchaseReturnForm').on('submit', function(e) {
            if ($('#itemRows tr.item-row').length == 0) {
                e.preventDefault();
                errorMessage('Please select a service purchase with returnable items.');
                return false;
            }
