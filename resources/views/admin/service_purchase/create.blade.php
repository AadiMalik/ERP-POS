...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4">{{ isset($service_purchase) ? 'Update' : 'New' }} Service Purchase</h4>
        <div class="card">
            <div class="card-header bg-white border-bottom">
                <h5 class="mb-0">{{ isset($service_purchase) ? 'Update' : 'Create' }} Service Purchase</h5>
            </div>
            <div class="card-body">
                <form action="{{ url('admin/service-purchase') }}" method="POST" id="servicePurchaseForm">
                    @csrf
                    <input type="hidden" name="service_purchase_id" value="{{ $service_purchase->service_purchase_id ?? '' }}">
                    {{-- ================= HEADER ================= --}}
                    <div class="row">
                                ...
                                <select class="form-control select2" name="business_id" id="business_id">
                                    ...
                                    @foreach ($business as $item)
                                        <option value="{{ $item->business_id }}"
                                            {{ old('business_id', $service_purchase->business_id ?? '') == $item->business_id ? 'selected' : '' }}>
                                            {{ $item->code }} - {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <select class="form-control select2" name="supplier_id" id="supplier_id">
                                ...
                                @foreach ($suppliers as $item)
                                    <option value="{{ $item->supplier_id }}"
                                        {{ old('supplier_id', $service_purchase->supplier_id ?? '') == $item->supplier_id ? 'selected' : '' }}>
                                        {{ $item->code }} - {{ $item->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-3 mb-3">
                            <label>Service Purchase No.</label>
                            <input type="text" class="form-control" name="service_purchase_no" readonly
                                value="{{ $service_purchase->service_purchase_no ?? ($service_purchase_no ?? 'Auto Generated') }}">
                        </div>
                        <div class="col-md-3 mb-3">
                        ...
                        <div class="col-md-12 mb-3">
                            ...
                            <textarea class="form-control" rows="2" name="description">{{ old('description', $service_purchase->description ?? '') }}</textarea>
                        </div>
                    </div>
                    <hr>
                    {{-- ================= ITEM TABLE ================= --}}
...
@section('js')
    ...
    <script>
        ...
        @endforeach
        var isEditMode = {{ isset($service_purchase) ? 'true' : 'false' }};
        var editItems = @json($service_purchase_details['items'] ?? []);
                ...
                <tr class="item-row">
                    ...
                    <td>
                        <input type="text" class="form-control item-name" name="items[${index}][item_name]"
                            value="${item.item_name ?? ''}" placeholder="e.g. Gas Cylinder Rental">
                    </td>
                    <td>
