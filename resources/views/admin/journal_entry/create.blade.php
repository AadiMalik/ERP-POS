...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="card">
        ...
        <form id="journal_entryForm">
            @csrf
            <input type="hidden" name="journal_entry_id" id="journal_entry_id" value="{{ isset($journal_entry) ? $journal_entry->journal_entry_id : '' }}">
            <div class="card-body">
                {{-- ================= MASTER ================= --}}
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <select class="form-select select2" id="journal_id" name="journal_id">
                            ...
                            <option value="{{ $journal->journal_id }}" {{isset($journal_entry)? $journal_entry->journal_id == $journal->journal_id ? 'selected' : '' : ''}}>{{ $journal->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Entry No<span class="text-danger">*</span></label>
                        <input readonly class="form-control" value="{{ isset($journal_entry) ? $journal_entry->entry_no : '' }}" id="entry_no" name="entry_no">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Entry Date<span class="text-danger">*</span></label>
                        <input type="date" class="form-control" id="entry_date" name="entry_date" value="{{ isset($journal_entry) ? $journal_entry->entry_date : date('Y-m-d') }}">
                    </div>
                    <div class="col-md-4 mb-3">
                        <label class="form-label">Reference No<span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="reference_no" name="reference_no" value="{{isset($journal_entry) ? $journal_entry->reference_no : ''}}">
                    </div>
                    <div class="col-md-8">
                        <label class="form-label">Description<span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="description" name="description" value="{{isset($journal_entry) ? $journal_entry->description : ''}}">
                    </div>
                </div>
                <hr>
                {{-- ================= DETAILS ================= --}}
                    ...
                    <div class="offset-md-8 col-md-4">
                        <div class="total-box">
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <input type="text" disabled id="difference" class="difference-success total-input" value="0.00">
                            </div>
                        </div>
                    </div>
                </div>
