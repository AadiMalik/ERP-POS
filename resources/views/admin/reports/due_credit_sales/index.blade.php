...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4">{{ __('reports.due_credit_sales') }}</h4>
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <div class="d-flex gap-2">
                    @canAccess('reports.due-credit-sales.print')
                    <a href="javascript:void(0);" id="btn_print" class="btn btn-outline-secondary">
                        <i class="fa fa-print"></i> {{ __('common.print') }}
                    </a>
                    @endcanAccess
                    @canAccess('reports.due-credit-sales.pdf')
                    <a href="javascript:void(0);" id="btn_pdf" class="btn btn-outline-danger">
                        <i class="fa fa-file-pdf"></i> {{ __('common.pdf') }}
                    </a>
                    @endcanAccess
                    @canAccess('reports.due-credit-sales.export')
                    <a href="javascript:void(0);" id="btn_excel" class="btn btn-outline-success">
                        <i class="fa fa-file-excel"></i> {{ __('common.excel') }}
                    </a>
                    @endcanAccess
                    @canAccess('reports.due-credit-sales.export-csv')
                ...
                <div class="row g-3 p-4 pb-0">
                    <div class="col-md-3">
                        <div class="alert alert-info mb-0">
                            <strong>Orders with Due:</strong> <span id="grand_orders_display">-</span>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="alert alert-secondary mb-0">
                            <strong>Total Amount:</strong> <span id="grand_total_display">-</span>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="alert alert-success mb-0">
                            <strong>Paid:</strong> <span id="grand_paid_display">-</span>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="alert alert-warning mb-0">
                            <strong>Total Due:</strong> <span id="grand_due_display">-</span>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="alert alert-secondary mb-0">
                            <strong>Customers with Dues:</strong> <span id="grand_customers_display">-</span>
                        </div>
                    </div>
                </div>

                <div class="table-responsive p-4">
                    <table id="due_credit_sales_table" class="table datatables">
                        <thead>
                            <tr>
...
@section('js')
    @include('admin.partials.datatable', [
        ...
        'route' => 'due-credit-sales/data',
        'buttons' => false,
        'pageLength' => 50,
        'notordering' => true,
        'class' => 'due_credit_sales_table',
        'variable' => 'due_credit_sales_table',
        'datefilter' => true,
        'params' => "business_id:$('#business_id').val(),branch_id:$('#branch_id').val(),order_source_id:$('#order_source_id').val(),status:$('#status').val(),user_id:$('#user_id').val()",
    ])
    ...
    <script>
        ...
        $('#search_btn').click(function() {
            initDataTabledue_credit_sales_table();
            refreshTotals();
        });

        function refreshTotals() {
            $.ajax({
                url: url_local + '/admin/reports/due-credit-sales/data',
                type: 'POST',
                data: $.extend({
                ...
                success: function(response) {
                    ...
                    $('#grand_paid_display').text(response.grand_paid ?? '-');
                    $('#grand_due_display').text(response.grand_due ?? '-');
                    $('#grand_customers_display').text(response.grand_customers ?? '-');
                }
            });
        }

        $('#btn_print').click(function() {
            window.open(buildReportUrl('/admin/reports/due-credit-sales/print'), '_blank');
        });
        $('#btn_pdf').click(function() {
            window.open(buildReportUrl('/admin/reports/due-credit-sales/pdf'), '_blank');
        });
        $('#btn_excel').click(function() {
            window.location.href = buildReportUrl('/admin/reports/due-credit-sales/export');
        });
        $('#btn_csv').click(function() {
            window.location.href = buildReportUrl('/admin/reports/due-credit-sales/export-csv');
        });
    </script>
@endsection
