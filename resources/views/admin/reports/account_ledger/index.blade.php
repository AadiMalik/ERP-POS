...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4">{{ __('reports.account_ledger') }}</h4>
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <div class="d-flex gap-2">
                    @canAccess('reports.account-ledger.print')
                    <a href="javascript:void(0);" id="btn_print" class="btn btn-outline-secondary">
                        <i class="fa fa-print"></i> {{ __('common.print') }}
                    </a>
                    @endcanAccess
                    @canAccess('reports.account-ledger.pdf')
                    <a href="javascript:void(0);" id="btn_pdf" class="btn btn-outline-danger">
                        <i class="fa fa-file-pdf"></i> {{ __('common.pdf') }}
                    </a>
                    @endcanAccess
                    @canAccess('reports.account-ledger.export')
                    <a href="javascript:void(0);" id="btn_excel" class="btn btn-outline-success">
                        <i class="fa fa-file-excel"></i> {{ __('common.excel') }}
                    </a>
                    @endcanAccess
                    @canAccess('reports.account-ledger.export-csv')
            ...
            <div class="card-body">
                            ...
                            <select id="account_id" class="form-select">
                                ...
                                @foreach ($accounts as $item)
                                    <option value="{{ $item->account_id }}"
                                        {{ request('account_id') == $item->account_id ? 'selected' : '' }}>
                                        {{ $item->code ?? '' }} {{ $item->name ?? '' }}
                                    </option>
                                @endforeach
                            </select>
                ...
                <div class="row g-3 p-4 pb-0">
                    ...
                    <div class="col-md-4">
                        <div class="alert alert-info mb-0">
                            <strong>Total Debit / Credit:</strong> <span id="total_debit_display">-</span> /
                            <span id="total_credit_display">-</span>
                        </div>
                    </div>
                    <div class="col-md-4">
                ...
                <div class="table-responsive p-4">
                    <table id="account_ledger_table" class="table datatables">
                        <thead>
                            <tr>
...
@section('js')
    @include('admin.partials.datatable', [
        ...
        'route' => 'account-ledger/data',
        'buttons' => false,
        'pageLength' => 25,
        'notordering' => true,
        'class' => 'account_ledger_table',
        'variable' => 'account_ledger_table',
        'datefilter' => true,
    ...
    <script>
        ...
        $('#search_btn').click(function() {
            initDataTableaccount_ledger_table();
            refreshTotals();
        });

        function refreshTotals() {
            $.ajax({
                url: url_local + '/admin/reports/account-ledger/data',
                type: 'POST',
                data: $.extend({
        ...
        $('#btn_print').click(function() {
            window.open(buildReportUrl('/admin/reports/account-ledger/print'), '_blank');
        });
        $('#btn_pdf').click(function() {
            window.open(buildReportUrl('/admin/reports/account-ledger/pdf'), '_blank');
        });
        $('#btn_excel').click(function() {
            window.location.href = buildReportUrl('/admin/reports/account-ledger/export');
        });
        $('#btn_csv').click(function() {
            window.location.href = buildReportUrl('/admin/reports/account-ledger/export-csv');
        });
    </script>
@endsection
