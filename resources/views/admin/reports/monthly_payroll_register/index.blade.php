...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4">{{ __('reports.monthly_payroll_register') }}</h4>
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <div class="d-flex gap-2">
                    @canAccess('reports.monthly-payroll-register.print')
                    <a href="javascript:void(0);" id="btn_print" class="btn btn-outline-secondary">
                        <i class="fa fa-print"></i> {{ __('common.print') }}
                    </a>
                    @endcanAccess
                    @canAccess('reports.monthly-payroll-register.pdf')
                    <a href="javascript:void(0);" id="btn_pdf" class="btn btn-outline-danger">
                        <i class="fa fa-file-pdf"></i> {{ __('common.pdf') }}
                    </a>
                    @endcanAccess
                    @canAccess('reports.monthly-payroll-register.export')
                    <a href="javascript:void(0);" id="btn_excel" class="btn btn-outline-success">
                        <i class="fa fa-file-excel"></i> {{ __('common.excel') }}
                    </a>
                    @endcanAccess
                    @canAccess('reports.monthly-payroll-register.export-csv')
            ...
            <div class="card-body">
                ...
                <div class="table-responsive p-4">
                    <table id="monthly_payroll_register_table" class="table datatables">
                        <thead>
                            <tr>
...
@section('js')
    @include('admin.partials.datatable', [
        'columns' => "
                        ...
                        {data:'leave_days',name:'leave_days',sortable:false,className:'text-end'},
                        {data:'status',name:'status',sortable:false}",
        'route' => 'monthly-payroll-register/data',
        'buttons' => false,
        'pageLength' => 50,
        'notordering' => true,
        'class' => 'monthly_payroll_register_table',
        'variable' => 'monthly_payroll_register_table',
        'params' => "business_id:$('#business_id').val(),month:$('#month').val(),year:$('#year').val(),department_id:$('#department_id').val(),branch_id:$('#branch_id').val()",
    ])
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#business_id, #month, #year, #department_id, #branch_id').select2();
            refreshTotals();
        });

        $('#search_btn').click(function() {
            initDataTablemonthly_payroll_register_table();
            refreshTotals();
        });

        function refreshTotals() {
            $.ajax({
                url: url_local + '/admin/reports/monthly-payroll-register/data',
                type: 'POST',
                data: $.extend({
        ...
        $('#btn_print').click(function() {
            window.open(buildReportUrl('/admin/reports/monthly-payroll-register/print'), '_blank');
        });
        $('#btn_pdf').click(function() {
            window.open(buildReportUrl('/admin/reports/monthly-payroll-register/pdf'), '_blank');
        });
        $('#btn_excel').click(function() {
            window.location.href = buildReportUrl('/admin/reports/monthly-payroll-register/export');
        });
        $('#btn_csv').click(function() {
            window.location.href = buildReportUrl('/admin/reports/monthly-payroll-register/export-csv');
        });
    </script>
@endsection
