        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="table-responsive p-4">
                    <table id="serial_number_customer_table" class="table datatables">
                        <thead>
                            <tr>
                                ...
                                <th>{{ __('reports.col_warranty_until') }}</th>
                            </tr>
                        </thead>
                    </table>
                </div>
...
@section('js')
    @include('admin.partials.datatable', [
        'columns' => "
                        ...
                        {data:'warranty_expires_at',name:'warranty_expires_at',sortable:false}",
        'route' => 'serial-number-customer/data',
        'buttons' => false,
        'pageLength' => 25,
        'notordering' => true,
        'class' => 'serial_number_customer_table',
        'variable' => 'serial_number_customer_table',
        'params' => "business_id:$('#business_id').val(),customer_id:$('#customer_id').val(),product_id:$('#product_id').val(),product_variation_id:$('#product_variation_id').val(),serial_no:$('#serial_no').val()",
    ])
    <script>
        function currentReportParams() {
            let p = {};
            ['business_id', 'customer_id', 'product_id', 'product_variation_id', 'serial_no'].forEach(function(id) {
        ...
        $('#search_btn').click(function() { serial_number_customer_table.ajax.reload(); });
        $('#reset_filter').click(function() {
            $('#filterSection select').val('').trigger('change');
            $('#filterSection input').val('');
            serial_number_customer_table.ajax.reload();
        });
        $('#toggleFilter').click(function() { $('#filterSection').slideToggle(); });
        $('#btn_print').click(function() { window.open(buildReportUrl('/admin/reports/serial-number-customer/print'), '_blank'); });
        $('#btn_pdf').click(function() { window.open(buildReportUrl('/admin/reports/serial-number-customer/pdf'), '_blank'); });
        $('#btn_excel').click(function() { window.location = buildReportUrl('/admin/reports/serial-number-customer/export'); });
        $('#btn_csv').click(function() { window.location = buildReportUrl('/admin/reports/serial-number-customer/export-csv'); });
    </script>
@endsection
