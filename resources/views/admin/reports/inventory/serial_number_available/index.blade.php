        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <div class="d-flex gap-2">
                    @canAccess('reports.serial-number-available.print')
                    <a href="javascript:void(0);" id="btn_print" target="_blank" class="btn btn-outline-secondary">
                        <i class="fa fa-print"></i> Print
                    </a>
                    @endcanAccess
                    @canAccess('reports.serial-number-available.pdf')
                    <a href="javascript:void(0);" id="btn_pdf" target="_blank" class="btn btn-outline-danger">
                        <i class="fa fa-file-pdf"></i> PDF
                    </a>
                    @endcanAccess
                    @canAccess('reports.serial-number-available.export')
                    <a href="javascript:void(0);" id="btn_excel" class="btn btn-outline-success">
                        <i class="fa fa-file-excel"></i> Excel
                    </a>
                    @endcanAccess
                    @canAccess('reports.serial-number-available.export-csv')
            ...
            <div class="card-body">
                ...
                <div class="table-responsive p-4">
                    <table id="serial_number_available_table" class="table datatables">
                        <thead>
                            <tr>
                                <th>Serial No.</th>
                                <th>Product</th>
...
@section('js')
    @include('admin.partials.datatable', [
        ...
        'route' => 'serial-number-available/data',
        'buttons' => false,
        'pageLength' => 25,
        'notordering' => true,
        'class' => 'serial_number_available_table',
        'variable' => 'serial_number_available_table',
        'params' => "business_id:$('#business_id').val(),warehouse_id:$('#warehouse_id').val(),product_id:$('#product_id').val(),product_variation_id:$('#product_variation_id').val(),serial_no:$('#serial_no').val()",
    ])
    <script>
        function currentReportParams() {
            let p = {};
            ['business_id', 'warehouse_id', 'product_id', 'product_variation_id', 'serial_no'].forEach(function(id) {
        ...
        $('#search_btn').click(function() { serial_number_available_table.ajax.reload(); });
        $('#reset_filter').click(function() {
            $('#filterSection select').val('').trigger('change');
            $('#filterSection input').val('');
            serial_number_available_table.ajax.reload();
        });
        $('#toggleFilter').click(function() { $('#filterSection').slideToggle(); });
        $('#btn_print').click(function() { window.open(buildReportUrl('/admin/reports/serial-number-available/print'), '_blank'); });
        $('#btn_pdf').click(function() { window.open(buildReportUrl('/admin/reports/serial-number-available/pdf'), '_blank'); });
        $('#btn_excel').click(function() { window.location = buildReportUrl('/admin/reports/serial-number-available/export'); });
        $('#btn_csv').click(function() { window.location = buildReportUrl('/admin/reports/serial-number-available/export-csv'); });
    </script>
@endsection
