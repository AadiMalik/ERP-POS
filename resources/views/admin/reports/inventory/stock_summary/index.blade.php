        ...
        <div class="card">
            ...
            <div class="card-body">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-3">
                            ...
                            <select id="report_mode" class="form-select">
                                    <option value="summary">{{ __('reports.stock_summary') }}</option>
                                    <option value="availability">{{ __('reports.availability') }}</option>
                                    <option value="warehouse_branch">{{ __('reports.warehouse_branch') }}</option>
                                    <option value="low_stock">{{ __('reports.low_stock') }}</option>
                            </select>
                        </div>
                ...
                <div class="row g-3 p-4 pb-0">
                    ...
                    <div class="col-md-3">
                        <div class="alert alert-info mb-0">
                            <strong>Available:</strong> <span id="total_available_display">-</span>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="alert alert-info mb-0">
                            <strong>Value:</strong> <span id="total_value_display">-</span>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="alert alert-info mb-0">
                            <strong>Reorder:</strong> <span id="total_reorder_display">-</span>
                        </div>
                    </div>
                </div>
                <div class="table-responsive p-4">
                    <table id="stock_summary_table" class="table datatables">
                        <thead>
                            <tr>
                                ...
                                <th>{{ __('common.qty') }}</th>
                                <th>{{ __('reports.col_reserved') }}</th>
                                <th>{{ __('reports.col_available') }}</th>
                                <th>{{ __('reports.col_avg_price') }}</th>
                                <th>{{ __('reports.col_value') }}</th>
                                <th>{{ __('reports.col_min_stock') }}</th>
                                <th>{{ __('reports.col_reorder') }}</th>
                            </tr>
                        </thead>
                    </table>
                </div>
...
@section('js')
    @include('admin.partials.datatable', [
        'columns' => "
                        ...
                        {data:'reserved_quantity',name:'reserved_quantity',sortable:false,className:'text-end'},
                        {data:'available_quantity',name:'available_quantity',sortable:false,className:'text-end'},
                        ...
                        {data:'minimum_stock',name:'minimum_stock',sortable:false,className:'text-end'},
                        {data:'reorder_qty',name:'reorder_qty',sortable:false,className:'text-end'}",
        'route' => 'stock-summary/data',
        'buttons' => false,
        'pageLength' => 25,
        'notordering' => true,
        'class' => 'stock_summary_table',
        'variable' => 'stock_summary_table',
        
        'params' => "business_id:$('#business_id').val(),branch_id:$('#branch_id').val(),warehouse_id:$('#warehouse_id').val(),product_id:$('#product_id').val(),product_variation_id:$('#product_variation_id').val(),category_id:$('#category_id').val(),brand_id:$('#brand_id').val(),report_mode:$('#report_mode').val()",
    ])
    <script>
        ...
        $('#search_btn').click(function() { stock_summary_table.ajax.reload(); refreshTotals(); });
        ...
        $('#btn_print').click(function() { window.open(buildReportUrl('/admin/reports/stock-summary/print'), '_blank'); });
        $('#btn_pdf').click(function() { window.open(buildReportUrl('/admin/reports/stock-summary/pdf'), '_blank'); });
        $('#btn_excel').click(function() { window.location = buildReportUrl('/admin/reports/stock-summary/export'); });
        $('#btn_csv').click(function() { window.location = buildReportUrl('/admin/reports/stock-summary/export-csv'); });
        function refreshTotals() {
            setTimeout(function() {
                let json = stock_summary_table.ajax.json();
                if (!json) return;
                if (json.total_qty !== undefined) $('#total_qty_display').text(json.total_qty);
                if (json.total_available !== undefined) $('#total_available_display').text(json.total_available);
