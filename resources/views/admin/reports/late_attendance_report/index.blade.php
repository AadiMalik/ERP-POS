...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        <h4 class="fw-bold py-3 mb-4">{{ __('reports.late_attendance_report') }}</h4>
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                ...
                <div class="d-flex gap-2">
                    @canAccess('reports.late-attendance-report.print')
                    <a href="javascript:void(0);" id="btn_print" class="btn btn-outline-secondary">
                        <i class="fa fa-print"></i> {{ __('common.print') }}
                    </a>
                    @endcanAccess
                    @canAccess('reports.late-attendance-report.pdf')
                    <a href="javascript:void(0);" id="btn_pdf" class="btn btn-outline-danger">
                        <i class="fa fa-file-pdf"></i> {{ __('common.pdf') }}
                    </a>
                    @endcanAccess
                    @canAccess('reports.late-attendance-report.export')
                    <a href="javascript:void(0);" id="btn_excel" class="btn btn-outline-success">
                        <i class="fa fa-file-excel"></i> {{ __('common.excel') }}
                    </a>
                    @endcanAccess
                    @canAccess('reports.late-attendance-report.export-csv')
            ...
            <div class="card-body">
                ...
                <div class="row g-3 p-4 pb-0">
                    <div class="col-md-4">
                        <div class="alert alert-warning mb-0"><strong>Total Late Minutes:</strong> <span id="total_late_minutes_display">-</span></div>
                    </div>
                </div>

                <div class="table-responsive p-4">
                    <table id="late_attendance_report_table" class="table datatables">
                        <thead>
                            <tr>
...
@section('js')
    @include('admin.partials.datatable', [
        ...
        'route' => 'late-attendance-report/data',
        'buttons' => false,
        'pageLength' => 50,
        'notordering' => true,
        'class' => 'late_attendance_report_table',
        'variable' => 'late_attendance_report_table',
        'datefilter' => true,
    ...
    <script>
        ...
        $('#search_btn').click(function() {
            initDataTablelate_attendance_report_table();
            refreshTotals();
        });

        function refreshTotals() {
            $.ajax({
                url: url_local + '/admin/reports/late-attendance-report/data',
                type: 'POST',
                data: $.extend({
                ...
                success: function(response) {
                    $('#total_late_minutes_display').text(response.total_late_minutes ?? '-');
                }
            });
        }

        $('#btn_print').click(function() {
            window.open(buildReportUrl('/admin/reports/late-attendance-report/print'), '_blank');
        });
        $('#btn_pdf').click(function() {
            window.open(buildReportUrl('/admin/reports/late-attendance-report/pdf'), '_blank');
        });
        $('#btn_excel').click(function() {
            window.location.href = buildReportUrl('/admin/reports/late-attendance-report/export');
        });
        $('#btn_csv').click(function() {
            window.location.href = buildReportUrl('/admin/reports/late-attendance-report/export-csv');
        });
    </script>
@endsection
